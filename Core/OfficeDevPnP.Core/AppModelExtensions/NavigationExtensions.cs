...
namespace Microsoft.SharePoint.Client
{
	...
	public static partial class NavigationExtensions
	{
		...
		public static AreaNavigationEntity GetNavigationSettings(this Web web)
		{
			...
			else
			{
				...
				foreach (var node in navNodes)
				{
					...
					{
						LoadNavigationSettingsFromProvider(nav.CurrentNavigation, node);
					}
					else if (node.Attribute("Name").Value.Equals("GlobalNavigationTaxonomyProvider", StringComparison.InvariantCulture))
					{
						LoadNavigationSettingsFromProvider(nav.GlobalNavigation, node);
					}
				}
			}
		...
		public static void UpdateNavigationSettings(this Web web, AreaNavigationEntity navigationSettings)
		{
			...
			{
				webNav.GlobalNavigation.Source = Publishing.Navigation.StandardNavigationSource.InheritFromParentWeb;
			}
			else
			{
			...
			if (navigationSettings.CurrentNavigation.UseParentSiteMap)
			{
				webNav.CurrentNavigation.Source = Publishing.Navigation.StandardNavigationSource.InheritFromParentWeb;
			}
			else
			{
